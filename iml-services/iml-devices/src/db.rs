...
fn compute_virtual_device_changes<'a>(
    ...
    // for db_virtual_device in db_virtual_devices {
    //     tracing::info!("db_virtual_device: {:#?}", db_virtual_device);
    //     let db_virtual_device_host =
    //         incoming_device_hosts.get(&(db_virtual_device.id.clone(), fqdn.clone()));
    //     tracing::info!("db_virtual_device_host: {:#?}", db_virtual_device_host);
    ...
    //     for host in all_other_host_fqdns {
    //         let mut i = BreadthFirstIterator::new(incoming_devices, &db_virtual_device.id);
    //         let all_available = i.all(|p| {
    //             let result = db_device_hosts.get(&(p.clone(), host.clone())).is_some();
    //             tracing::info!("DB: Checking device {:?} on host {:?}: {:?}", p, host, result);
    //             result
    //         });
    //         tracing::info!(
    //             "DB: Host: {:#?}, device: {:#?}, all_available: {:?}",
    //             host,
    //             db_virtual_device.id,
    //             all_available
    //         );
    //         // We're only considering removal here. All devices that could've been added are added above
    //         if !all_available {
    //             // We might not need this check. We're iterating DB virtual devices in the first place
    //             if db_device_hosts
    //                 .get(&(db_virtual_device.id.clone(), host.clone()))
    //                 .is_some()
    //             {
    //                 let other_device_host = DeviceHost {
    //                     device_id: db_virtual_device.id.clone(),
    //                     fqdn: host.clone(),
    //                     local: true,
    //                     paths: Paths(BTreeSet::new()),
    //                     mount_path: MountPath(None),
    ...
    //                 tracing::info!(
    //                     "DB: Removing device host with id {:?} on host {:?}",
    //                     db_virtual_device.id,
    //                     host,
    //                 );
    //                 results.insert(
    //                     (db_virtual_device.id.clone(), host.clone()),
    //                     Change::Remove(other_device_host),
    //                 );
    //             } else {
    //                 // It wasn't in the DB in the first place, nothing to do
